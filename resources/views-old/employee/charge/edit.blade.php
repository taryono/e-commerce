        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-horizontal" method="POST" action="{{ route('charge.update', $charge->id) }}"  enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <input type="hidden" class="form-control" name="_method" value="PUT">
                        <div class="form-group{{ $errors->has('amount') ? ' has-error' : '' }}">
                            <label for="amount" class="col-md-4 control-label">Jumlah</label> 
                            <div class="col-md-6">
                                <input id="amount" type="text" class="form-control" name="amount" value="{{ $charge->amount }}" required autofocus>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <select name="from_province_id" class="form-control">
                                    ...
                                    <option value="{{$c->id}}" {{($c->id == $charge->from_province_id)?'selected="selected"':''}}>{{ucfirst($c->name)}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <select name="to_province_id" class="form-control">
                                    @foreach($provinces as $c)
                                    <option value="{{$c->id}}" {{($c->id == $charge->to_province_id)?'selected="selected"':''}}>{{ucfirst($c->name)}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
